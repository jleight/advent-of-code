...
{
    public class D04P1 : SolutionBase
    {
        private readonly static Regex _parser = new Regex(@"\[(\d+)-(\d+)-(\d+) (\d+):(\d+)\] (.+)");
        private readonly static Regex _guard = new Regex(@"Guard #(\d+) begins shift");
